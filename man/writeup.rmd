...
So now, after the lucky run, the ~ 96% accuracy doesn’t look too bad. Submit the result, it does score well.

Does story end here now? Of course not. As we are scientist, we study and research. KNN is one of the many classification algorithms. The next one on the list is random forest, and see who performs better for this particular data set. Regression tree is another one on the list, so does naive bayes.
...
##Considerations
With limited time, only able to try out regression tree and random forest. From the confusion matrix shown below, the results look way more encouraging than KNN.
